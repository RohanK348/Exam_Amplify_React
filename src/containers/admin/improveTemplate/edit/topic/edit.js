...
import { AddToPhotosOutlined, UnfoldMore } from "@material-ui/icons";
...
const EditDialog = (props) => {
  ...
  const {item, index, complexityTypes, brain, topics, more} = props
  ...
  const validate = () => {
    ...
    if (!res) NotificationManager.warning(msg, "Worning", 3000);
    return res;
  }
  const handleSave = () => {
    if (!validate())
      return
    let templateTopic = item
    templateTopic.topicID = topics[topic].id
    templateTopic.topic = topics[topic]
    templateTopic.number = number
    templateTopic.complexity = complexity
    templateTopic.min = min
    templateTopic.avg = avg
    templateTopic.max = max
    if (item.brain === "right") {
      const topics = [...improveTemplate?.rightTopics];
      topics[index] = templateTopic
      dispatch({ type: "SET", name: "rightTopics", value: topics });
    } else if (item.brain === "left") {
      const topics = [...improveTemplate?.leftTopics];
      topics[index] = templateTopic;
  ...
  return (
    <>
      <AddToPhotosOutlined className="cursor-pointer" onClick={handleClickOpen} />
      <Dialog 
