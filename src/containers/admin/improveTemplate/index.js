...
import {getByPaginationFilter as getTemplates, update} from '../../../api/improveTemplate'
        ...
        <div>
          ...
          >
            <Box p={2}>
              <List dense={true}>
                <ListItem>
                  <Link to={`/admin/improveTemplate/${item.id}`} style={{textDecoration: 'none'}}>
                    <ListItemText
                      primary="Edit"
                    />
                  </Link>
...
const ImproveTemplate = () => {
  ...
  const refresh = (isSleep=true) => {
    ...
    setTemplates([])
    setFrom(limit)
    run(getTemplates({departmentID: {eq: id}, corporateID: {eq: ''}}, limit, 0))
    setAsyncState('get')
    setPending(true)
  }
  ...
  const loadMore = () => {
    setFrom(from+limit)
    run(getTemplates({departmentID: {eq: id}, corporateID: {eq: ''}}, limit, from))
    setAsyncState('get')
  }
  ...
  return (
    <>
      ...
      <Widget
        ...
        <Create refresh={refresh} departmentId={id} />
