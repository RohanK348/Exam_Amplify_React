          ...
          >
            <Box p={2}>
              ...
              <ListItem>
                ...
                <ListItem>
                  <Link to={`/admin/simpleTemplate/${item.rightID}`} style={{textDecoration: 'none'}}>
                    <ListItemText
                      primary="Right Brain"
                    />
                  </Link>
                ...
                <ListItem>
                  <Link to={`/admin/simpleTemplate/${item.leftID}`} style={{textDecoration: 'none'}}>
                    <ListItemText
                      primary="Left Brain"
                    />
                  </Link>
...
const SimpleTemplate = () => {
  ...
  const refresh = (isSleep=true) => {
    ...
    setTemplates([])
    setFrom(limit)
    run(getTemplates({isSimple: {eq: true}, corporateID: {eq: ''}}, limit, 0))
    setAsyncState('get')
    setPending(true)
  }
  ...
  const loadMore = () => {
    setFrom(from+limit)
    run(getTemplates({isSimple: {eq: true}, corporateID: {eq: ''}}, limit, from))
    setAsyncState('get')
  }
