...
const Student = () => {
  ...
  return (
    <>
      ...
      <SectionTitle title="Students Result" subtitle={`Student Result Manage of ${index*10} ~ ${index*10+10}%`} />
      <Widget
                      ...
                      return (
                        <TableCell key={column.id} align={column.align} style={{fontSize: 14, padding: '0px 10px'}}>
                          {column.id === 'action'?
                            (
                              <>
                                <Link to={`/admin/exam/student/detail/${examId}/${row?.user?.id}`} style={{textDecoration: 'none'}}>
