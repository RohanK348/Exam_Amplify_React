...
const Exam = () => {
  ...
  const loadMore = () => {
    run(getExams({openingID: {eq: ''}, isTrain: {eq: false}, type: {ne: 'PRIVATE'}}, limit, from))
    setFrom(from+limit)
    setPending(true)
  }
  ...
  useEffect(() => {
    setExams([])
    setFrom(limit)
    run(getExams({openingID: {eq: ''}, isTrain: {eq: false}, or: [{type: {ne: 'PRIVATE'}}, {type: {eq: 'PRIVATE'}}]}, limit, 0))
    setPending(true)
  }, [run])
  useEffect(() => {
    if (status === 'resolved') {
  ...
  return (
    <>
      ...
      <SectionTitle title="Empowerr Exam" subtitle="Empowerr Exam management" />
      <Widget
        title="Empowerr Exam"
        description={
          <span>
