...
const Exam = (props) => {
  ...
  const loadMore = () => {
    run(getExams({openingID: {eq: id}, activate: {eq: true}, isTrain: {eq: false}, type: {ne: 'PRIVATE'}}, limit, from, setting?.auth?.id))
    setFrom(from+limit)
    setPending(true)
  ...
  useEffect(() => {
    if (id && setting.auth) {
      setExams([])
      setFrom(limit)
      run(getExams({openingID: {eq: id}, activate: {eq: true}, isTrain: {eq: false}, or: [{type: {ne: 'PRIVATE'}}, {type: {eq: 'PRIVATE'}, students: {eq: setting?.auth?.id}}]}, limit, 0, setting?.auth?.id))
      setPending(true)
      setAsyncState('getExams')
    }
  }, [run, id, setting.auth, limit])
  useEffect(() => {
    if (status === 'resolved') {
      if (asyncState === 'getExams') {
        if (opening?.corporate?.verified && opening?.corporate?.candidateNumber >= -100) {
