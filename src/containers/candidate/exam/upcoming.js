...
const Exam = () => {
  ...
  const loadMore = () => {
    run(getExams({activate: {eq: true}, isTrain: {eq: false}, type: {ne: 'PRIVATE'}}, limit, from, setting?.auth?.id))
    setFrom(from+limit)
    setPending(true)
  ...
  useEffect(() => {
    if (setting.auth) {
      setExams([])
      setFrom(limit)
      run(getExams({activate: {eq: true}, isTrain: {eq: false}, or: [{type: {ne: 'PRIVATE'}}, {type: {eq: 'PRIVATE'}, students: {eq: setting?.auth?.id}}]}, limit, 0, setting?.auth?.id))
      setPending(true)
      setAsyncState('getExams')
    }
