...
const Result = () => {
  ...
  const loadMore = () => {
    run(getExamResults({userID: {eq: setting?.auth?.id}, corporateID: {eq: ''}, isTrain: {eq: false}}, limit, from))
    setFrom(from+limit)
    setPending(true)
  }
  ...
  useEffect(() => {
    if (setting?.auth) {
      setExamResults([])
      setFrom(limit)
      run(getExamResults({userID: {eq: setting?.auth?.id}, corporateID: {eq: ''}, isTrain: {eq: false}}, limit, 0))
      setPending(true)
    }
  }, [run, setting?.auth])
  useEffect(() => {
