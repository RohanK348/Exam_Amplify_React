...
const columns = [
  { id: 'examName', label: 'Template Name', minWidth: 150, align: 'center' },
...
const Result = () => {
  ...
  const loadMore = () => {
    run(getExamResults({userID: {eq: setting?.auth?.id}, isTrain: {eq: true}}, limit, from))
    setFrom(from+limit)
    setPending(true)
  }
  ...
  useEffect(() => {
    if (setting?.auth) {
      setExamResults([])
      setFrom(limit)
      run(getExamResults({userID: {eq: setting?.auth?.id}, isTrain: {eq: true}}, limit, 0))
      setPending(true)
    }
  }, [run, setting?.auth])
  useEffect(() => {
