...
import {seHistory} from 'react-router-dom'
...
const Final = (props) => {
  ...
  const handleAccept = () => {
    setModalActive(false)
    history.push('/corporate/exam/create')
  }
        ...
        <Grid item lg={7} md={10} xs={12}>
          <Grid container spacing={2} style={{padding: '50px 10px'}}>
            <Grid item md={5} xs={12} className={customeClasses.image} >
              <img alt="image" src={`${process.env.PUBLIC_URL}/images/corporate/image_1.png`} style={{width: 250}} />
            </Grid>
