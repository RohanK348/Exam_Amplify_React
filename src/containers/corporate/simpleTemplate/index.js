          ...
          >
            <Box p={2}>
              ...
              <ListItem>
                ...
                {/* <ListItem>
                  <Link to={`/corporate/template/${item.id}`} style={{textDecoration: 'none'}}>
                    <ListItemText
                      primary="Detail"
                    />
                  </Link>
                ...
                <ListItem>
                  <Link to={`/corporate/simpleTemplate/${item.rightID}`} style={{textDecoration: 'none'}}>
                    <ListItemText
                      primary="Right Brain"
                    />
                  </Link>
                ...
                <ListItem>
                  <Link to={`/corporate/simpleTemplate/${item.leftID}`} style={{textDecoration: 'none'}}>
                    <ListItemText
                      primary="Left Brain"
                    />
                  </Link>
...
const SimpleTemplate = () => {
  ...
  const refresh = (isSleep=true) => {
    ...
    setTemplates([])
    setFrom(limit)
    run(getTemplates({isSimple: {eq: true}, corporateID: {eq: setting?.auth?.corporateID}}, limit, 0))
    setAsyncState('get')
    setPending(true)
  }
  ...
  const loadMore = () => {
    setFrom(from+limit)
    run(getTemplates({isSimple: {eq: true}, corporateID: {eq: setting?.auth?.corporateID}}, limit, from))
    setAsyncState('get')
  }
  ...
  return (
    <>
      ...
      <SectionTitle title="Simple Template" subtitle="Simple Template management" />
      <Widget
        ...
        <Create refresh={refresh} corporateId={setting?.auth?.corporateID} />
